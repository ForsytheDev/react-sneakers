...
const CartBoard = ({ openCartBoard, removeCartItem }) => {
  const { cartCollection, setCartCollection, setOrderCollection, setOpenCartBoard, itemLoader } =
    useContext(Storage)
  ...
  const addOrderItem = async () => {
    setOrderCollection((prev) => [...prev, ...cartCollection])
    setCartCollection([])
    setIsOrdered(true)
  }
  ...
  return (
    <div
      className={cn('cart-board', {
        open: openCartBoard,
      })}
    >
      <div className="cart-board-overflow" onClick={() => setOpenCartBoard(false)}></div>
      <div className="cart">
        ...
        {cartCollection.length > 0 ? (
          ...
          <div className="cart-quick-buy">
            <FillerBullet
              ...
              describe={
                isOrdered
                  ? `Ваш заказ был успешно передан нашему менеджеру!`
